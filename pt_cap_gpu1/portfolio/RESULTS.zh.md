...
**1. Prefill 的 V²f 律（`P=P₀+κT(1+ρT)²`）在全部 workload 上成立**，R² 0.956–0.995
（含 Qwen3：0.99+）。跨模型、架构、prompt 长度、batch 全部一条凸 V²f。
...
|---|---|---|
| R²（功率空间，全部点） | 0.44–0.93（摘要为负） | **0.90–0.997**（9/10） |
| 相对 RMSE | 3.3–12.1% | **0.6–3.2%**（除 classify 9.8%） |
| 留一交叉验证 | 9.3–21.2% | **0.5–18.2%**，8/10 胜 |
...
**3. 天花板量级 `T_max = B·BW_eff/(权重 + B·C_eff·kv/tok)` 跨 150×（6.4→900 tok/s）成立**
