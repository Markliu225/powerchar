# 混合 workload 的功率封顶经济性 —— 论文经济模型（V100 / RTX 5090-MOCK / H200）
...
结论（λ=20% 为代表）：**三个平台 capping 都显著占优且盈利**——

- **V100**：Φ_CAP $485M vs TDP $305M / MW·5yr，末端利润 **1.59 倍**，额外投资 **~1.2 个月**回本。
- **RTX 5090**（⚠ **MOCK 数据**，[data_5090/](../data_5090/)，无实测）：$512M vs $365M，**1.40 倍**，
  **~0.9 个月**回本。
- **H200**：Φ_CAP $369M vs TDP $307M，**1.20 倍**，约 **10.3 个月**回本。
...
> - **第一组** [`fig_profit_model.png`](fig_profit_model.png)：3 行（V100 / RTX 5090-MOCK / H200）× 4 列（价格年降率
>   λ = 0/10/20/30 %/yr）的**累计净现金流**曲线，CAP vs TDP；
> - **第二组** [`fig_profit_mix.png`](fig_profit_mix.png)：1×3（V100 / RTX 5090-MOCK / H200），λ 固定 20%，
...
`{v100,5090,h200}/workload_rack_capping.csv`。
...
|---|--:|--:|--:|--:|--:|--:|
| **V100** | 780 / 492 | 609 / 384 | 485 / 305 | 393 / 247 | 1.2 mo | **1.59** |
| **RTX 5090**（MOCK）| 820 / 585 | 641 / 458 | 512 / 365 | 417 / 297 | 0.9 mo | **1.40** |
| **H200** | 628 / 515 | 478 / 395 | 369 / 307 | 289 / 242 | 9.5–10.9 mo | **1.19–1.22** |

- **G 对 λ 稳健**：衰减近似等比压低两条曲线，三个平台的 G 在整个 λ 扫描内变动不超过几个百分点。
- **T× 对 λ 的敏感度随回收期变长而上升**：V100/5090 回收在 ~1 个月（价格几乎未衰减），λ 扫描下不动；
  H200 回收要 ~10 个月，落进价格衰减区间，λ 从 0 到 30% 把它从 9.5 推到 10.9 个月。
- **G 随卡价递减（1.59 > 1.40 > 1.20）**：便宜卡上"多装卡"的杠杆更大——V100 CAP 多装 60% 的卡、
  ΔK 仅 $6.0M；H200 折旧占比高摊薄相对增益。绝对量：每 MW 5 年多赚 V100 **$180M** ·
  5090 **$147M** · H200 **$62M**。
...
|---|--:|--:|--:|
| **V100** | $6.0M | 1.2 mo（1.2–1.3） | **+$180M**（带 [+173, +187]） |
| **RTX 5090**（MOCK）| $3.6M | 0.9 mo | **+$147M**（带 [+144, +150]） |
| **H200** | $21.2M | 10.3 mo（10.1–10.5） | **+$62M**（带 [+60, +65]） |
...
- **按设备标定**：$c_g$ 与吞吐/功耗数据。**全组共用**：$e$、$\beta$、$\mu$、$n$、按类价 $\pi^{(j)}$；
  λ 是第一组列变量（第二组固定 20%）。
- **取值**：$c_g=$ **\$2.5k(V100)/\$3.5k(5090)/\$27k(H200)** · $n=$ **5 年**、$S=0$ · $e=$ **\$0.10/kWh** ·
  $\beta=$ **1.1** · $\mu=$ **4%** · 集群 **1 MW**（200 / 87 / 71 机架）· ΔK = **$6.0M / $3.6M / $21.2M** ·
  `PRICE_SCALE` = **1/3**（挂牌分档价的小模型折价，见 §6 第 3 条；置 1.0 即恢复挂牌价）。

## 6. 局限

- **RTX 5090 一行是 MOCK 数据**：其吞吐/功耗曲线由 H200 拟合按 5090 规格比合成
  （[data_5090/](../data_5090/)，无任何实测），$c_g$=$3.5k 也是假设——该行只作展望，
  图题与脚注均已标注，引用其数字必须带 MOCK 声明。
